...
#Parameter
bgtnummer = arcpy.GetParameterAsText(0) #Configure parameter: Input, Text, Required
...
def createEmptyLayer(bgtnummer):
    #Nieuwe layer voor huidige ArcPro sessie
    arcpy.AddMessage("Nieuwe laag creëren")
    layername = rf"Bevindingen_{bgtnummer}"
    layer = arcpy.management.CreateFeatureclass(
        out_path=gdb,
        out_name=layername,
        template=r"G:\civ\IGA_ATG\Producten\BGT\Toetstooling\datamodel_template\datamodel_template.shp",
        geometry_type="POINT",
